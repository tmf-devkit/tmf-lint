...
async def _ensure_orders(client: LintClient, count: int) -> bool:
    for _ in range(count):
        resp = await client.post(
            ...
            json={
                ...
                "description": f"tmf-lint-pag-{uuid.uuid4().hex[:8]}",
                "orderItem": [
                    {
...
class TMF641LimitQueryParam(BaseRule):
    """GET /serviceOrder?limit=1 returns at most 1 item."""

    rule_id = "TMF641-PAG-001"
    api = 641
    category = CATEGORY_PAGINATION
    description = "GET /serviceOrder?limit=1 returns at most 1 item"

    async def check(self, client: LintClient, ctx: LintContext) -> RuleResult:
        try:
            await _ensure_orders(client, _MIN_SEED)
            resp = await client.get(_ORDER, params={"limit": 1})
...
class TMF641XTotalCountMatchesActual(BaseRule):
    ...
    async def check(self, client: LintClient, ctx: LintContext) -> RuleResult:
        ...
        if total_count == 0:
            return self.fail("X-Total-Count=0 after seeding orders — unexpected")

        # ── Step 2: fetch limit=total and confirm body length matches ─────────
        try:
            full_resp = await client.get(_ORDER, params={"limit": total_count})
        except Exception as exc:
...
class TMF641OffsetBeyondTotalReturnsEmpty(BaseRule):
    """GET /serviceOrder?offset=999999 returns 200 with empty array."""

    rule_id = "TMF641-PAG-003"
    api = 641
    category = CATEGORY_PAGINATION
    description = "GET /serviceOrder?offset=999999 returns 200 with empty array"

    async def check(self, client: LintClient, ctx: LintContext) -> RuleResult:
        try:
            resp = await client.get(_ORDER, params={"offset": 999_999, "limit": 10})
        except Exception as exc:
            return self.fail(f"Request failed: {exc}")
